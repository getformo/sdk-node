# Agent setup for `sdk-node`

This is a deepsec scanning workspace. Project `sdk-node` was just registered
(target: `..`). Setup is incomplete — `data/sdk-node/INFO.md`
...
2. **Fill in `data/sdk-node/INFO.md`.** It's auto-injected into the AI
...
```bash
pnpm deepsec scan    --project-id sdk-node
pnpm deepsec process --project-id sdk-node
```
