...
Currently configured project: `sdk-node` (target: `..`).
...
3. Open the parent repo in your coding agent (Claude Code, Cursor, …)
   and have it follow `data/sdk-node/SETUP.md` to fill in
   `data/sdk-node/INFO.md`.
...
`--project-id sdk-node` (or whichever id you want) explicitly.

`scan` is free (regex only). `process` is the AI stage (≈$0.30/file
on Opus by default). Run state goes to `data/sdk-node/`.
